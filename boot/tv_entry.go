...
const (
	...
	TvEntryDescription = "Internal RK entry which implements tv web with Echo framework."
)
...
func readFileFromPkger(filePath string) []byte {
	...
	if file, err = pkger.Open(path.Join("github.com/rookie-ninja/rk-echo:/boot", filePath)); err != nil {
		return []byte{}
	}
...
func (entry *TvEntry) AssetsFileHandler() echo.HandlerFunc {
...
// @Router /rk/v1/tv/ [get]
func (entry *TvEntry) TV(ctx echo.Context) error {
	if ctx == nil {
		return fmt.Errorf("nil echo context")
	}
