...
type ParseTokenFunc func(auth string, ctx echo.Context) (*jwt.Token, error)
...
func newOptionSet(opts ...Option) *optionSet {
	...
	if reflect.ValueOf(set.Skipper).Pointer() == reflect.ValueOf(defaultSkipper).Pointer() {
		set.Skipper = func(ctx echo.Context) bool {
...
func (set *optionSet) defaultParseToken(auth string, ctx echo.Context) (*jwt.Token, error) {
	token := new(jwt.Token)
	var err error
